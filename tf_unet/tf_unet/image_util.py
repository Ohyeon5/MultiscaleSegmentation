...
class ImageDataProvider(BaseDataProvider):
    ...
    # Overwite _process_data and _process_labels
    # To control input image size and label variances
    # Set all the input image size to 400 x 400
    # Label images should be masks (0 or 1)
    def _process_data(self, data, im_size=300):
        # normalization
        data = np.clip(np.fabs(data), self.a_min, self.a_max)
        data -= np.amin(data)
        data /= np.amax(data)
        return misc.imresize(data, (im_size, im_size))

    def _process_labels(self, label, im_size=300):
        nx = label.shape[1]
        ny = label.shape[0]
        labels = np.zeros((im_size, im_size, self.n_class), dtype=np.float32)
        if self.n_class == 2:
            labels[..., 1] = 1*misc.imresize(label, (im_size, im_size)).astype(bool)
            labels[..., 0] = 1*(~misc.imresize(label, (im_size, im_size)).astype(bool))
        elif self.n_class == 21:
            for ii in range(self.n_class):
                index = np.where(label==ii+1)
                temp = np.zeros((ny,nx))
                temp[index[0], index[1]] = 1
                if index[0].size:
                    labels[...,ii] = misc.imresize(temp, (im_size, im_size)).astype(bool)
