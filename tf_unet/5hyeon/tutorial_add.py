...
cwd = os.getcwd()
output_path = cwd + '/output/tutorial_add/iter20_alpha1e-4/'

#setup & training
# input: 3 channels (RGB), features_root:#layer in the first layer, layers: # of layers
net = unet.Unet(layers=5, features_root=64, channels=3, n_class=21, integration="add")
trainer = unet.Trainer(net,optimizer="adam", opt_kwargs=dict(learning_rate=1e-4))
path = trainer.train(data_provider, output_path, training_iters=20, epochs=100, prediction_path=output_path + 'prediction')


test_data_provider = image_util.ImageDataProvider("./data/testDB/*", n_class=21, data_suffix=".jpg", mask_suffix="_mask.png")
test_data, test_label = test_data_provider._load_data_and_label
path = cwd + '/output/tutorial_add/img/'
prediction = net.predict(path, test_data)
unet.error_rate(prediction, util.crop_to_shape(test_label, prediction.shape))
img = util.combine_img_prediction(test_data, test_label, prediction)
