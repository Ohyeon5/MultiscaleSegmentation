...
class Reporter:
    ...
    def save_image(self, train, test, epoch,fnames):
        if epoch < 1000:
            ...
            if fnames:
                file_name = str(epoch)+fnames+self.IMAGE_EXTENSION
            else:
                file_name = str(epoch)+len(glob.glob(self._image_test_dir))+self.IMAGE_EXTENSION
            test_filename = os.path.join(self._image_test_dir, file_name)
            test.save(test_filename)

    def save_image_from_ndarray(self, train_set, test_set, palette, epoch, index_void=None,fnames=None):
        train_image=[]
        test_image=[]
        if len(train_set):
        ...
        self.save_image(train_image, test_image, epoch, fnames)
...
class MatPlotManager:
    ...
    def add_figure(slf, title, xylabels, labels, filename=None):
