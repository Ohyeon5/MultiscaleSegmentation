...
class UNet:
    ...
    def create_model(size, l2_reg):
        ...
        # combined1 = tf.math.maximum(UNet.conv_transpose(conv5_2, filters=512, l2_reg_scale=l2_reg), conv4_2)
        ...
        # conv_up1_1 = UNet.conv(combined1, filters=512, l2_reg_scale=l2_reg)
        conv_up1_2 = UNet.conv(conv_up1_1, filters=512, l2_reg_scale=l2_reg)
        # combined2 = tf.math.add(UNet.conv_transpose(conv_up1_2, filters=256, l2_reg_scale=l2_reg), conv3_2)
        ...
        # conv_up2_1 = UNet.conv(combined2, filters=256, l2_reg_scale=l2_reg)
        conv_up2_2 = UNet.conv(conv_up2_1, filters=256, l2_reg_scale=l2_reg)
        # combined3 = UNet.integ_softendmax(UNet.conv_transpose(conv_up2_2, filters=128, l2_reg_scale=l2_reg), conv2_2)
        ...
        # conv_up3_1 = UNet.conv(combined3, filters=128, l2_reg_scale=l2_reg)
        conv_up3_2 = UNet.conv(conv_up3_1, filters=128, l2_reg_scale=l2_reg)
        # combined4 = UNet.integ_softendmax(UNet.conv_transpose(conv_up3_2, filters=64, l2_reg_scale=l2_reg), conv1_2)
        ...
        # conv_up4_1 = UNet.conv(combined4, filters=64, l2_reg_scale=l2_reg)
    ...
    @staticmethod
    def integ_softendmax(convTransposed, conved, weight=0.5):
        # by Oh-hyeon
        difference = tf.subtract(conved, convTransposed)
        combined = tf.math.add(convTransposed, tf.multiply(difference, tf.multiply(weight,tf.cast(tf.greater(difference,0),tf.float32))))
        return combined
